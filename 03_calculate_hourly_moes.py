...
OUTPUT_FILE = RESULTS_DIR / "link_moe_hourly.csv"
...
# Calculate slow movements and DSH for selected links
def calculate_speed_moes(data, selected_links):
    ...
    # Consecutive speed changes only within the same link run
    ...
    # Summarize each continuous visit to the link
    run_dsh = (
        selected.groupby(
            [
                "trip_segment_id",
    ...
    # DSH needs at least two waypoint speeds
    run_dsh = run_dsh[
        run_dsh["waypoint_count"] >= 2
    ].copy()

    # Combine multiple visits to the same link within the same trip
    trip_dsh = (
        run_dsh.groupby(
            [
                "trip_segment_id",
    ...
    # Paper definition: sum of speed changes divided by n waypoint speeds
    trip_dsh["trip_dsh"] = (
        trip_dsh["speed_change_sum"]
        / trip_dsh["waypoint_count"]
    )

    # Link DSH = average of trip-level DSH values
    dsh = (
        trip_dsh.groupby(
            "link_key",
            as_index=False,
...
# Calculate travel time only for fully observed link traversals
def calculate_travel_times(data, selected_links):
    ...
    # Keep only complete traversals within the same trip segment
    complete = runs[
...
# Combine chunk results into one hourly record per link
def combine_results(
    basic_parts,
    dsh_parts,
    travel_parts,
...
def main():
    ...
    print("HOURLY LINK MOE PROCESSING")
    print("=" * 70)
    ...
    for chunk_number, chunk in enumerate(
        ...
        # Step 4: Carry final journey into the next chunk
        carry = chunk[
            chunk["journey_id"]
            == last_journey
        ].copy()
        ...
        # Step 6: Calculate slow movements and DSH
    ...
    print()
    print("=" * 70)
    print("HOURLY MOE PROCESSING COMPLETE")
    print(f"Selected links: {len(selected):,}")
