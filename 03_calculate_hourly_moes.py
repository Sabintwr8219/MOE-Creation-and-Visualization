...
def waypoint_metrics(batch, selected_links):
    ...
    return (
        valid.groupby("link_key", observed=True)
        .agg(
...
def dsh_metrics(batch, selected_links):
    ...
    groups = ["link_key", "journey_id", "trip_segment", "run_id"]
    ...
    return (
        trip.groupby("link_key", observed=True)
...
def travel_time_metrics(batch, selected_links):
    ...
    complete = complete[complete["travel_time_s"].gt(0)]
    if complete.empty:
        return pd.DataFrame()

    return (
        complete.groupby("link_key", observed=True)
        .agg(
...
def combine(frames, values):
    ...
    if not frames:
        return pd.DataFrame()
    return pd.concat(frames, ignore_index=True).groupby("link_key", observed=True)[values].sum().reset_index()


def process_batch(batch, selected_links):
    batch = prepare_batch(batch)
    return waypoint_metrics(batch, selected_links), dsh_metrics(batch, selected_links), travel_time_metrics(batch, selected_links)
...
def main():
    ...
    result = waypoint.merge(dsh, on="link_key", how="left").merge(travel, on="link_key", how="left")
